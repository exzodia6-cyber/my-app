...
export function ItemCard({ item, selected, onClick, action }: { item: Item | InventoryItem; selected?: boolean; onClick?: () => void; action?: React.ReactNode }) {
  ...
  return <div onClick={onClick} className={`group rounded-3xl border bg-slate-950/70 p-4 transition hover:-translate-y-1 hover:border-cyan-300/60 ${selected ? 'border-gold shadow-neon' : 'border-white/10'}`}>
    ...
    <div className="mt-4 flex items-start justify-between gap-3"><div><p className="font-bold">{data.weapon}</p><p className="text-sm text-slate-300">{data.skin}</p></div><p className="text-gold font-black">{data.price.toLocaleString('ru-RU')} монет</p></div>
    <p className="mt-2 text-xs text-slate-400">{rarityLabel[data.rarity]} · {data.wear.replaceAll('_',' ').toLowerCase()}</p>{action && <div className="mt-3">{action}</div>}
  </div>
}
